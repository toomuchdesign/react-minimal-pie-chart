...
| **segmentsShift**     | _Number_ (%), _Function_              | Translates segments radially. If `number`set provide percentage of chart's radio. If `function` return a value for each segment: `(data, dataIndex) => number` </br>(`props.radius` value might need to be adjusted to prevent segments from overflowing chart's boundaries) | 0          |
